...
import { addAddress } from "../../redux/slices/ordersSlice";
      ...
      <address className="flex flex-row gap-3 items-center justify-between">
        <section className="flex flex-row gap-5 items-center">
          <input defaultChecked={address.addressDefault === true} id="default-radio-2" type="radio" value="" name="default-radio" 
            className="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-none" onClick={() => dispatch(addAddress(address?._id)) }
          />
          <span className="font-roboto font-medium text-xl text-gray-600">{address?.firstName} {address?.lastName}</span>
          <span className="font-roboto text-sm text-gray-500 mt-1 ml-2">{address?.country}, {address?.city}, {address?.streetAddressAndNumber}, {address?.zipCode} </span>
        </section>
